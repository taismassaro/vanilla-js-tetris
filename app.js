...
/**
 * alternative way of putting the script tag
 * in the head of the html file and only
 * run it after the DOM loads, instead of 
 * adding the tag to the bottom of the html
**/
document.addEventListener('DOMContentLoaded', () => {
  ...
  function moveLeft() {
    ...
    const isLeftCellBlocked = currentShape.some(index => gridCells[currentPosition + index - 1].classList.contains('taken'));
  ...
  function moveRight() {
    ...
    const isRightCellBlocked = currentShape.some(index => gridCells[currentPosition + index + 1].classList.contains('taken'));
  ...
  startButton.addEventListener('click', () => {
    if (timerId) {
      ...
      drawShape()
      nextPieceRandomIndex = Math.floor(Math.random() * shapes.length);
